...
# Performs feature selection on data in the form of a dataframe.
# The data is transformed so that the resulting features are 
# principal components are explain a specified percentage of total variance.
#
...
#         standardized (set to true if each feature in df has 0 mean and unit
#                       variance)

def feature_selection_pca(df, co_var=0.9, co_feat=None, standardized=False):
    
    if standardized == False: df = feature_normalization(df)
    ...
    for var in pca.explained_variance_ratio_:
        n_component += 1
        var_sum += var
        if var_sum > co_var or n_component >= co_feat: break
        
    # create dataframe with principal components as features   
    df_new = pd.DataFrame()    
